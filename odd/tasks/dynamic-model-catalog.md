...
- [x] T14 Zen free models excluded (2026-09-26): with a Zen API key, `muse-spark-1.3-contributor-free` still returned `FreeTierError` (user report), so free models are only served to OpenCode's own app. `zenFetcher` drops `-free` ids and `big-pickle`; seed no longer lists `big-pickle`; the unavailable-model detector also matches "free tier can only be used from within OpenCode". `bun test` 63 pass, `bunx tsc -b` exit 0.
- [x] T15 Web search verified live (2026-09-26) with the user's keys: Groq browser_search works on gpt-oss-120b, gpt-oss-20b, gpt-oss-safeguard-20b (qwen3.8-27b rejects tools other than function/mcp). OpenCode Go searched on gpt-5.6-luna, gpt-6-luna, grok-4.6, grok-4.7 (responses, `web_search`), minimax-m3 (messages, `web_search_20250305`), hy3, hy4-preview, kimi-k2.6, kimi-k3, mimo-v2.5 (chat, `web_search_preview`); others answer without searching or reject the tool. Zen could not be tested: account balance empty (HTTP 402) and some models "Model access is disabled". With a key, Zen `/models` returns the workspace list (50 ids) including internal `test` and `test-novita-dsf4.1`, now excluded. Changes: `webSearch.ts` (Go set, gpt-oss-20b), `groqTools.ts` (gpt-oss-20b tool-capable), Go chat tool type `web_search_preview`, `zenFetcher` excludes `test*`. `bun test` 77 pass, `bunx tsc -b` exit 0. Uncommitted.
- [ ] T16 (next) Context limits and reasoning effort from models.dev: `getModelTokenLimit` shows a generic 8192 for most OpenCode Go/Zen models and underestimates 1M-context models (see engram `web-chat/model-limits-effort`). Plan: extend `scripts/update-vision-models.ts` to also generate `limit.context`, `limit.output` and `reasoning_options`, then add an effort selector showing only the levels each model accepts.
